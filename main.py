...
def api_buchhaltung_request():
    ...
    if ROLE_BUCHHALTUNG_ID not in user_roles:
        return jsonify({"success": False, "message": "Nur die Buchhaltung kann diesen Endpunkt nutzen."}), 403
        
    data = request.get_json(silent=True) or {}
    message = safe_str(data.get("message"), "")[:1000]
    
    if not message:
        return jsonify({"success": False, "message": "Anfrage darf nicht leer sein."}), 400
        
    # Anforderung speichern und automatisch als Aufgabe ins System hinterlegen
    now = now_utc()
    req_doc = {
    ...
    buchhaltung_requests_collection.insert_one(req_doc)
    
    task_doc = {
        "title": "Neue Anfrage aus der Buchhaltung",
        "type": "Allgemein",
        "priority": "medium",
        "description": f"Nachricht von {session['user']['username']}: {message}",
        "status": "open",
        "created_at": now,
        "assignee": None
    }
